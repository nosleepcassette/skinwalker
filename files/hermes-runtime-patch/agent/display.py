...
def get_skin_wings() -> list[tuple[str, str]]:
    """Get spinner wing pairs from the active skin."""
    skin = _get_skin()
    if skin:
...
def decorate_with_skin_wings(text: str, *, index: int = 0) -> str:
    """Wrap a text snippet with the active skin's wing pair when available."""
    wings = get_skin_wings()
    if not wings or not text:
        return text
    left, right = wings[index % len(wings)]
